<template>
  <section class="w-full min-h-screen bg-gray-100 flex flex-col items-center justify-center px-20">
    ...
    <div v-if="ongoingEvents.length"
      ...
      <h1
        class="mb-10 text-3xl font-extrabold leading-none tracking-tight text-gray-700 md:text-3xl lg:text-4xl dark:text-white">
        ongoing</h1>
      <div class="w-full bg-gray-100 flex gap-10 flex-row flex-wrap items-center justify-start">
        <div v-for="event in ongoingEvents" :key="event.uuid"
          class="transition duration-150 ease-in-out ]top-0 left-0 s w-10/12 md:w-1/3 shadow-2xl">
    ...
    <div v-for="eventType in eventTypes"
      ...
      <div v-if="events.length" class="w-full bg-gray-100 flex gap-10 flex-row flex-wrap items-center justify-start">
        <div v-for="event in events"
          class="transition duration-150 ease-in-out top-0 left-0 w-10/12 md:w-1/3 shadow-2xl">
          <div v-if="event.event_type == eventType" class="w-full" :key="event.uuid">
...
export default {
  ...
  methods: {
    ...
    async getUpcomingEvents() {
      const response = await $fetch('http://127.0.0.1:8000/events/get/upcoming/', {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json'
        },
    ...
    async getOngoingEvents() {
      const response = await $fetch('http://127.0.0.1:8000/events/get/ongoing/', {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json'
        },
